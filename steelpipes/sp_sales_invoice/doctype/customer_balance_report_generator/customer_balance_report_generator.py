...
def generate_customer_balance(company=None,customer_group=None,territory=None,sales_person=None,balance_less_than=None,get_advances=0,all_balances=0):
	...
	now = datetime.now()
	file_name = get_file_name(sales_person,balance_less_than,get_advances,all_balances)		
	...
	merge_right = workbook.add_format({'align': 'right','bold': True, 'font': 'Comic Sans MS', 'font_size':14})
	merge_right.set_border()
	merge_right.set_align('vcenter')
	merge_center = workbook.add_format({'align': 'center','bold': True, 'font': 'Comic Sans MS', 'font_size':14})
	merge_center.set_border()
	merge_center.set_align('vcenter')

	datestr = 'Date: ' + str(now)
	worksheet.write(0,0, datestr, cell_format_font_14)
	partystr = ''
	if customer_group:
	...
	cell_format = workbook.add_format({'align': 'center','bold': True, 'font': 'Comic Sans MS', 'font_size':14})
	cell_format.set_border()
	cell_format.set_align('vcenter')
	worksheet.write(2,0, 'Parties', cell_format)
	worksheet.write(2,4, 'Parties', cell_format)
	cell_format = workbook.add_format({'align': 'center','bold': True, 'font': 'Comic Sans MS', 'font_size':9})
	cell_format.set_border()
	cell_format.set_align('vcenter')
	...
	cell_format = workbook.add_format({'align': 'center','bold': True, 'font': 'Arial', 'font_size':9})
	cell_format.set_align('vcenter')
	cell_format.set_border()
	...
	# Adding up balances
	customer_list = get_customer_balance_list(company,customer_group,territory,sales_person,balance_less_than,get_advances,all_balances)
	cell_col = 0
	cell_format_arial = workbook.add_format({'align': 'center','bold': True, 'font': 'Arial', 'font_size':10, 'num_format': '#,##,###'})
	cell_format_arial.set_align('vcenter')
	cell_format_arial.set_border()
	cell_format_arial_date = workbook.add_format({'align': 'center','bold': True, 'font': 'Arial', 'font_size':10})
	...
	cell_format_comic_sans_ms = workbook.add_format({'align': 'left','bold': True, 'font': 'Comic Sans MS', 'font_size':10})
	...
	for i in customer_list:
		...
		if sales_person:
			sql_query_str = '''SELECT parent,sales_person FROM `tabSales Team` WHERE parent='{0}' AND sales_person='{1}';'''.format(i.name,sales_person)
...
def get_file_name(sales_person,balance_less_than,get_advances,all_balances):
	...
	if all_balances == 0 and get_advances == 0:
		filestr = '/tmp/{0}customer_balance_{1}.xlsx'.format(sp_str,balance_less_than)
	elif all_balances == 0 and get_advances == 1:
...
def get_customer_balance_list(company=None,customer_group=None,territory=None,sales_person=None,balance_less_than=None,get_advances=0,all_balances=0):
	customer_list = None
	customer_list = frappe.get_list('Customer',filters={'Disabled':'No'},page_length= 100000)

	for customer in customer_list:
		outstanding_balance = get_balance_on(date=date.today(),party_type='Customer',party=customer.name,company=company)
		frappe.db.set_value('Customer',customer.name,'outstanding_balance',outstanding_balance)
	
	sql_query_str = get_filters(company,customer_group,territory,sales_person,balance_less_than,get_advances,all_balances)
...
def generate_xlsx_customer_balance():
	...
	file_path = get_file_name(doc.sales_person,doc.balance_less_than,doc.get_advances,doc.all_balances)
	file_name = get_download_file_name(doc.sales_person,doc.balance_less_than,doc.get_advances,doc.all_balances)
...
def get_download_file_name(sales_person,balance_less_than,get_advances,all_balances):
	...
	if sales_person:
		...
		sp_str = ''
	if all_balances == 0 and get_advances == 0:
		filestr = '{0}customer_balance_{1}.xlsx'.format(sp_str,balance_less_than)
	elif all_balances == 0 and get_advances == 1:
